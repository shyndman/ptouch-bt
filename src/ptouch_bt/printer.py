...
ImagePath: TypeAlias = str | bytes | Path | BinaryIO
ImageSource: TypeAlias = PILImage | ImagePath
FontPath: TypeAlias = str | Path
...
def rasterline_setpixel(rasterline: bytearray, pixel: int) -> None:
  if pixel < 0 or pixel >= len(rasterline) * 8:
    return
  rasterline[(len(rasterline) - 1) - (pixel // 8)] |= 1 << (pixel % 8)


def packbits_raster_line(rasterline: bytearray) -> bytes:
  return bytes([0x47, len(rasterline) + 1, 0x00, len(rasterline) - 1]) + bytes(
    rasterline
  )


def load_image(source: ImageSource) -> PILImage:
...
def coerce_image_fit(fit: ImageFit | str) -> ImageFit:
  if isinstance(fit, ImageFit):
    return fit
  return ImageFit(fit)


def grayscale_image(image: PILImage) -> PILImage:
...
def resize_to_fit_tape(
  image: PILImage, tape_width: int, fit: ImageFit | str = ImageFit.CONTAIN
) -> PILImage:
  if tape_width < 1:
  ...
  if fit is ImageFit.NONE:
    raise ValueError(
      f"image height {height}px exceeds tape printable width {printable_height}px"
    )

  scale = printable_height / height
  resized_width = max(1, round(width * scale))
  resize = cast(Callable[[tuple[int, int], int], PILImage], image.resize)
  return resize((resized_width, printable_height), Image.Resampling.LANCZOS)


def monochrome_image(
  image: PILImage, *, dither: bool = True, threshold: int = 128
) -> PILImage:
  ...
  point = cast(Callable[[Callable[[int], int], str], PILImage], image.point)
...
def image_to_raster_chunks(image: PILImage) -> Iterator[bytes]:
  ...
  pixels = bitmap.load()
  if pixels is None:
    raise RuntimeError("bitmap pixels are unavailable")
...
def build_print_job(
  image: PILImage, finalize: FinalizeMode = FinalizeMode.FEED_CUT
) -> PrintJob:
  chunks = (
    INIT_COMMAND,
    PACKBITS_COMMAND,
  ...
  return PrintJob(chunks=chunks, image=image, finalize=finalize)
...
def build_image_print_job(
  ...
  *,
  ...
  return build_print_job(
    prepare_image(source, tape_width, fit=fit, dither=dither, threshold=threshold),
    finalize,
  )


def write_print_job(fd: int, chunks: Iterable[bytes]) -> tuple[int, int]:
  byte_count = 0
  chunk_count = 0
  for chunk in chunks:
    write_all(fd, chunk)
...
def build_text_print_job(
  ...
  return build_print_job(
    render_text_image(text, tape_width, font_path, font_size, margin), finalize
  )
...
def render_text_image(
  text: str,
  tape_width_px: int,
  font_path: FontPath | None = DEFAULT_FONT,
  font_size: int = DEFAULT_TEXT_FONT_SIZE,
  margin: int = 4,
) -> PILImage:
  if not text:
  ...
  if tape_width_px <= margin * 2:
    raise ValueError(
      f"tape width {tape_width_px}px leaves no room after {margin}px margins"
    )

  resolved_font_path = Path(font_path) if font_path is not None else DEFAULT_FONT
  ...
  image = Image.new("1", (int(text_width + margin * 2), tape_width_px), 1)
  draw = ImageDraw.Draw(image)
...
def tape_width_px(media_width: int) -> int:
